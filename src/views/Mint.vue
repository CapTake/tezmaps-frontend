...
const mint = async () => {
        const { bytes } = prepareOperation({ op: 'mint', tick: props.tick, amt: limit.value })
        console.log(bytes)
    ...
    try {
        ...
        if (subscribe) {
            ...
            await api.collection('protocol_tickets').subscribe(id, (message) => {
                ...
                minted.value = new BigNumber(total_supply).dividedBy(new BigNumber(decimals.value)).toNumber()
                console.log(minted.value)
            }).catch(e => {
                console.log(e)
            })

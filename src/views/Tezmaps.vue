<template>
    <div class="mx-auto w-full text-center p-0">
        ...
        <div class="w-full px-5">
            ...
            <h3 class="pt-6 dark:text-slate-500">Tezoscriptions aren't yet supported by wallets. Inspect yours at <router-link to="/balance" class="transition-all underline hover:text-blue-500">Holdings</router-link>.</h3>
        </div>
    </div>
    <div class="fixed w-full h-0 bottom-0 z-50">
        <div class="py-5 px-6 w-full sm:max-w-xs rounded-t-xl absolute bg-white dark:bg-black dark:border-lightgrey dark:border-b-0 z-50 top-0 border border-slate-100 shadow-2xl transition-all duration-300 ease-out text-center" :class="{ '-translate-y-full': selection.length > 0 }">
            ...
            <button @click="claimTezmap(selection)" class="rounded mx-auto flex gap-1 justify-center items-center px-6 py-1.5 mb-6 bg-main bg-gradient-to-r from-lightblue to-darkblue text-light font-light text-lg" :class="{'hover:bg-blue-600 bg-blue-500': !minting, 'bg-slate-300 cursor-not-allowed': minting }">

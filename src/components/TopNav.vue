<template>
    <nav class="font-light font-customFont text-black dark:text-light">
        ...
        <div class="grow"></div>
        <div ref="target" @click="menu = false" class="md:flex md:justify-between gap-5 md:flex-row flex-col absolute md:static bg-white dark:bg-black top-16 right-4 rounded shadow-md md:shadow-none md:p-1 p-5" :class="{hidden: !menu, flex: menu}">
            <router-link v-if="isConnected" :to="{ name: 'users', params: { wallet: account.address } }" class="transition-all hover:text-blue-500">
        ...
        <!-- <SyncButton class="btn-primary mb-1 mt-1 btn-xs"/> -->
        <WalletButton/>
